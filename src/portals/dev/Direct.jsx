    ...
    <div className="page-shell grid gap-5 md:grid-cols-[320px_1fr]">
      ...
      <div className="card-surface flex flex-col h-[70vh]">
        ...
        <div className="border-t border-white/10 p-3">
          ...
          <input className="form-input flex-1" placeholder="Message…" value={text} onChange={(e)=>setText(e.target.value)} onKeyDown={(e)=> e.key==="Enter" && send()} disabled={!threadId || sending} />
          <button className="btn btn-primary" onClick={send} disabled={!threadId || !text.trim() || sending}>{sending ? "Sending..." : "Send"}</button>
          </div>
        </div>
      </div>
    </div>
