...
export default function Billings() {
  ...
  const visibleReceipts = useMemo(() => {
    const needle = query.trim().toLowerCase();
    return receipts.filter((receipt) => !needle || [receipt.receiptNumber, receipt.invoiceNumber, ...(receipt.invoiceNumbers || []), receipt.paymentId, ...(receipt.paymentIds || []), receipt.projectTitleSnapshot, receipt.clientSnapshot?.businessName, receipt.clientSnapshot?.contactName].filter(Boolean).join(" ").toLowerCase().includes(needle));
  }, [receipts, query]);
